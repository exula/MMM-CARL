...
class CatalogClient {
  constructor(account, { fetchImpl = globalThis.fetch, timeoutMs = 20000, maxPages = 100, debug = () => {}, baseUrl = BASE, configName = "default" } = {}) {
  ...
  async request(path, body) {
    ...
    try {
      ...
      const response = await this.fetch(url, {
        ...
        headers: { ...HEADERS, "Ls2pac-config-name": this.configName, ...(cookie ? { Cookie: cookie } : {}) },
  ...
  async login() {
    ...
    const { username, lastName, password = lastName } = this.account;
    const result = await this.request("/login?rememberMe=true", {
      username, ...(lastName ? { lastName } : {}), rememberMe: true, password
    });
    const cookies = await this.jar.getCookies(this.baseUrl + "/loans/0/20/Status");
...
const MESSAGES = {
  AUTH: "Login failed. Check the card and password against your library website; this library may use a different login flow.",
