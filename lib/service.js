...
class LoanService {
  constructor(accounts, { clientFactory, interval = 3600000, publish = () => {}, debug = () => {} } = {}) {
    this.debug = debug;
    this.entries = accounts.map((account, index) => ({ id: account.id, name: account.name, number: index + 1,
      client: clientFactory ? clientFactory(account) : new CatalogClient(account, { debug: (event, details) => debug(event, { ...details, account: index + 1 }) }),
